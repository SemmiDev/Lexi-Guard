...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json({ message: 'Invalid request data', details: error.errors }, { status: 400 });
    }
    console.error('Error saving history:', error);
    return NextResponse.json({ message: 'Error saving history', details: error.message }, { status: 500 });
  }
}
...
export async function GET(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Error fetching history', details: error.message }, { status: 500 });
  }
}
...
export async function DELETE(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Error deleting history', details: error.message }, { status: 500 });
  }
}
